...
public class MapMatrix {
	...
	/**
	 * Create a map based on the string provided
	 * Expected format: region;owner;armies
	...
	/**
 	 * Creates a matrix where array[x][y] = has_connection 
 	 * 1 = has_connection
 	 * 0 = no connection
 	 * Not using booleans because we use this for math
	 * @param regions amount of regions
	...
	public int[][] createSuperAdjacencyMatrix(Map full) {
		int num = full.getSuperRegions().size();
		int[][] matrix = new int[num][num];
		
		for(int i=0; i<num-1; i++) {
			for(int j=i+1; j<num-1; j++) {
				SuperRegion superA = fullMap.getSuperRegion(i);
				SuperRegion superB = fullMap.getSuperRegion(j);
	 ...
	 * @param degrees = assume 0 for non-diagonal values, so only one array is needed
	 * @param adjMatrix = assumed to be a square matrix
	 */
	...
	/**
	 * Calculates how many connections, or neighbors, all the regions have
	 * 1-d array because we can assume no connection (or 0) for all other values
	 * 
	 * array[i] = Sum all rows from adjMatrix
	 * Complexity: O(n^2) n=tiles
	 * @param regions amount of regions
	...
	/**
	 * Apparently you have to delete any n row and n column before taking the determinant
	 * I will just remove the last row and column
	 * @param lapMatrix int[n][n] the Laplacian Matrix to shift
	...
	public int[][] shiftLaplacianMatrix(int[][] lapMatrix) {
		...
		//fill it up all the way, so the last row/column in lapMatrix is left out
	...
	public void printArray(int[] array, String name) {
		File file = new File("/home/jalal/workspace/warlight2-engine/matrix/" + name + ".txt");
		try {
			if(file.exists())
				file.delete();
	...
	/**
	 * Prints to ../warlight2-engine/matrix/name.txt overwriting any previous file
	 * format: 'value, ' - one space after comma 
	 * @param matrix
	 */
